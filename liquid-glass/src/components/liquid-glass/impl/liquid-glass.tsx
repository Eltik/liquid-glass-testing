...
const LiquidGlass = forwardRef<HTMLDivElement, LiquidGlassProps>(({ children, width, height, className = "", style = {}, padding = "24px 32px", cornerRadius = 20, displacementScale = 25, blurAmount = 0.25, saturation = 110, brightness = 105, contrast = 120, initialPosition, draggable = true, minWidth = 100, minHeight = 50, ...props }, ref) => {
    ...
    const updateShader = useCallback(() => {
        ...
        if (feImage && feDisplacementMap) {
            ...
            feDisplacementMap.setAttribute("scale", (displacementScale / canvasDPI).toString());
        }
    }, [glassSize.width, glassSize.height, fragmentShader, canvasDPI, displacementScale]);
    ...
    return (
        <>
            ...
            <canvas ref={canvasRef} style={{ display: "none" }} width={Math.floor(glassSize.width * canvasDPI)} height={Math.floor(glassSize.height * canvasDPI)} />

            {/* SVG filter definition */}
            <svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" className="pointer-events-none fixed z-[9998]" style={{ transform: "translate3d(0, 0, 0)" }}>
                <defs>
                    <filter id={`${id.current}_filter`} filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB" x="0" y="0" width={glassSize.width.toString()} height={glassSize.height.toString()}>
                        <feImage ref={feImageRef} id={`${id.current}_map`} width={glassSize.width.toString()} height={glassSize.height.toString()} />
                        <feDisplacementMap ref={feDisplacementMapRef} in="SourceGraphic" in2={`${id.current}_map`} xChannelSelector="R" yChannelSelector="G" />
                    </filter>
                </defs>
            </svg>
