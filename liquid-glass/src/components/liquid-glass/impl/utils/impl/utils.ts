...
const generateId = (): string => `liquid-glass-${Math.random().toString(36).slice(2, 11)}`;
...
const constrainPosition = (x: number, y: number, width: number, height: number, offset = 10): { x: number; y: number } => {
...
export { smoothStep, length, roundedRectSDF, texture, generateId, getPaddingValues, constrainPosition };
