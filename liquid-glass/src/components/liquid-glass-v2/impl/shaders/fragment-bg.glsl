 ...
 * 
 * This shader is the first pass in the liquid glass multi-pass rendering pipeline.
 * It renders the background scene with shadow effects that anticipate where the
 * glass shapes will be placed in subsequent passes.
 * 
 * Key responsibilities:
 * - Render background content (images, videos, or test patterns)
 * - Generate soft shadows beneath glass shapes using signed distance fields
 * - Handle multiple background types with proper aspect ratio preservation
 * - Provide a clean foundation for blur and glass effect passes
 * 
 * Background types supported:
 * - Type 0: Subtle checkerboard pattern for testing
 * - Type 1: Quadrant-based pattern with different visual elements
 * - Type 2: Simple horizontal gradient
 * - Type 10+: Image/video textures with cover-style scaling
 * 
 * Shadow generation uses the same SDF calculations as the main glass pass
 * to ensure perfect alignment between shadows and the final glass shapes.
 * The shadows are rendered with exponential falloff for smooth, realistic edges.
 */

#version 300 es

// Use high precision for accurate shadow calculations
...
uniform float u_dpr;            // Device pixel ratio for crisp rendering

// Mouse interaction (for shadow positioning)
uniform vec2 u_mouse;           // Raw mouse position
uniform vec2 u_mouseSpring;     // Spring-smoothed mouse position
uniform float u_time;           // Animation time (currently unused)

// Glass shape definition uniforms
uniform float u_mergeRate;      // How smoothly shapes blend together
uniform float u_shapeWidth;     // Glass shape width in pixels
uniform float u_shapeHeight;    // Glass shape height in pixels
uniform float u_shapeRadius;    // Corner radius in pixels
uniform float u_shapeRoundness; // Corner curve smoothness factor

// Shadow generation uniforms
uniform float u_shadowExpand;   // How far shadow extends from shape edge
uniform float u_shadowFactor;   // Shadow opacity/intensity (0-1)
uniform vec2 u_shadowPosition;  // Shadow offset from shape center

// Background rendering uniforms
uniform int u_bgType;           // Background type selector (0-10+)
...
/*
 * Generate chessboard and bar patterns for testing backgrounds
 * 
 * Creates geometric patterns useful for testing glass effects against
 * high-contrast backgrounds. Helps visualize distortion and refraction.
 * 
 * @param uv - Screen coordinates in pixels
 * @param size - Pattern tile size in pixels
...
float chessboard(vec2 uv, float size, int mode) {
  // Generate bar patterns using step functions and modulo
  float yBars = step(size * 2.0, mod(uv.y * 2.0, size * 4.0));
  float xBars = step(size * 2.0, mod(uv.x * 2.0, size * 4.0));

  if (mode == 0) {
    return yBars;                    // Horizontal bars only
  } else if (mode == 1) {
    return xBars;                    // Vertical bars only
  } else {
 ...
 * 
 * Creates a horizontal split for testing glass effects against
 * high-contrast edges and gradients.
 * 
 ...
 * Higher 'n' values create more rectangular corners.
 * 
...
float roundedRectSDF(vec2 p, vec2 center, float width, float height, float cornerRadius, float n) {
  ...
  if (d.x > -cr && d.y > -cr) {
    // Corner region: use superellipse for customizable rounded corners
 ...
 * hard edges. Creates organic, flowing connections between shapes.
 * 
 ...
 * Kept for reference but not used in this implementation.
 * 
 * @param a - First SDF value
 * @param b - Second SDF value
